...
// SIGKILLs the wrapper's process group during unload. Its stdout/stderr are detached
// from the wrapper's pipes so that cmd.Wait() in llama-swap returns promptly.
func startDaemon(startCmd string, vllmURL string, healthPath string, waitTimeout time.Duration) error {
	...
	// Do NOT inherit the wrapper's stdout/stderr pipes. These are connected
	// to llama-swap's log collection; if the daemon holds them, cmd.Wait()
	// on the wrapper blocks until WaitDelay expires (10s extra on every
	// unload). The daemon logs to its own stdout which is now /dev/null —
	// vLLM logs are still visible through its own log file or docker logs.
